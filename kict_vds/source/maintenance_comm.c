...
extern uint8 kickRTSavingData_m();
extern int startflag;
uint32 histCount = 0;

uint8 recvFramePortA[MAX_MODEM_DATA_QUEUE_NO];
uint8 histFrame[100][17] ;
uint8 pktData[100];
...
void sendRealtimeIndivDataPacket_1(uint32 t_lane,uint8 reverse)
{
	int i;
	uint8 *tmptr;
	uint16 crc;
	uint32 speed_integer,sTemp;
	INDIV_DATA_RSP_PACKET_1 respPkt;
	u8 tmp,fUP;

	respPkt.head.ver = 0x01;
	respPkt.head.addr[0] = 'E';
 	respPkt.head.addr[1] = 'V';
 	respPkt.head.addr[2] = 'P';
 	respPkt.head.addr[3] = 'D';
 	respPkt.head.addr[4] = '0';
 	respPkt.head.addr[5] = '0';
 	respPkt.head.addr[6] = '0';
 	respPkt.head.addr[7] = '5';
	
	respPkt.head.opcode =  OP_REQUEST; //CMD_INDIVIDUAL_DATA_REQ;
	respPkt.head.len_hi =  0;
	respPkt.head.len_lo =  18; // 1+17

	respPkt.data.opcode = OP_DATA_SEND;

	tmp = t_lane+1; fUP  = 1;  // fUp = 1  up, 2 = down
	 if(tmp >2) {tmp = tmp-2; fUP = 2;}

	respPkt.data.lane =  tmp;
		histFrame[histCount][0] =  tmp;	 // lane

	respPkt.data.direct = fUP;
		histFrame[histCount][1] =   fUP; // direction

	// Time
	tmp = BIN2BCD(sys_time.wYear/100);
	respPkt.data.time.year_hi = tmp;
		histFrame[histCount][2] =   tmp; // year_hi
	tmp = BIN2BCD(sys_time.wYear%100);
	respPkt.data.time.year_lo = tmp;
		histFrame[histCount][3] =   tmp; // year_lo
	tmp = BIN2BCD(sys_time.wMonth);
	respPkt.data.time.month = tmp;
		histFrame[histCount][4] =   tmp; // month
	tmp  = BIN2BCD(sys_time.wDay);
	respPkt.data.time.day = tmp;
		histFrame[histCount][5] =   tmp; // date
	tmp = BIN2BCD(sys_time.wHour);
	respPkt.data.time.hour = tmp;
		histFrame[histCount][6] =   tmp; // hour
	tmp = BIN2BCD(sys_time.wMinute);
	respPkt.data.time.min = tmp;
		histFrame[histCount][7] =   tmp; // minute
	tmp = BIN2BCD(sys_time.wSecond);
	respPkt.data.time.sec_hi = tmp;
		histFrame[histCount][8] =   tmp; // second
	tmp = 0; //BIN2BCD((io_time_counter/10)%100);
	respPkt.data.time.sec_lo =  tmp; // ?????얜챷??癒?뵠 筌띾‘? 獄쎻뫗??? by jwank 090720 dddd
		histFrame[histCount][9] =   tmp; // msecond

	// speed
//	speed_integer = gstCurrData.speed[t_lane]/100;
	speed_integer = gstCurrData.speed[t_lane];
	
	respPkt.data.spd_hi = speed_integer/100;
		histFrame[histCount][10] = respPkt.data.spd_hi; // speed_hi
	if (reverse == TRUE) {
		respPkt.data.spd_hi |= 0x80;	// reverse 筌△뫀??
		histFrame[histCount][10] |=  0x80;
	}
	respPkt.data.spd_lo = speed_integer%100;
		histFrame[histCount][11] =   respPkt.data.spd_lo; // speed_lo
	// Occupy
	sTemp = gstCurrData.occupy[gstLaneLink[t_lane].s_loop];

	respPkt.data.lp_oc_hi = ((sTemp>>8 )& 0x0f);
		histFrame[histCount][12] =   sTemp/100; // occufied time_hi
	respPkt.data.lp_oc_lo = (sTemp & 0xff);
		histFrame[histCount][13] =   sTemp%100; // occufied time_lo
	// Length
	sTemp  = (gstCurrData.length[t_lane] );

	respPkt.data.len_hi = ((sTemp>>8)&0xff);
		histFrame[histCount][14] =   sTemp/100; // car length_hi
	respPkt.data.len_lo = (sTemp&0xff);
		histFrame[histCount][15] =  sTemp%100 ; // car length_lo
	respPkt.data.reserved = 0x00;
		histFrame[histCount][16] =   respPkt.data.reserved; // reserved
	...
	{
#if 0		
	     fprintf(stdout,"%02x",respPkt.head.ver);
	     for(i=0;i<8;i++) 	fprintf(stdout,"%02x",respPkt.head.addr[i] );
 
                  fprintf(stdout,"\n");                
 #else
		fprintf(stdout,"%02x%02x-%02x-%02x, %02x:%02x:%02x.%02x,%2d,%2d,%3d.%02d,%3d%02d,%3d%02d\n",
			histFrame[histCount][2],histFrame[histCount][3],histFrame[histCount][4],histFrame[histCount][5],
			histFrame[histCount][6],histFrame[histCount][7],histFrame[histCount][8],histFrame[histCount][9],
			histFrame[histCount][1],histFrame[histCount][0],histFrame[histCount][10],histFrame[histCount][11],
			histFrame[histCount][12],histFrame[histCount][13],histFrame[histCount][14],histFrame[histCount][15]);
#endif
 		fflush(stdout);
 	}

	if(startflag ==1) histCount++; // detect start
//	histCount++; // detect start

	int  flag_send = 0;
	send(fd_listener, (uint8 *)&respPkt, sizeof(respPkt), flag_send)  ;

	if(term_stat != REALTIME_MONITOR_MODE) 	fprintf(stdout, "packet send ...\n");	
//		fflush(stdout);
}
