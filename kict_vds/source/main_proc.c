...
{	
	...
	if(sys_time.wMilliseconds >= 100) { sys_time.wMilliseconds =0; sys_time.wSecond++; }
	if(sys_time.wSecond >= 60) {sys_time.wSecond = 0; sys_time.wMinute++;}
	if(sys_time.wMinute >= 60) {sys_time.wMinute = 0; sys_time.wHour++;}

   	global_counter+=10;
	if ( global_counter >= (3600000 /*/ _TICK_MS_*/) ) global_counter = 0; //by capi 2015.12.09 delete / _TICK_MS_. DCS9S _TICK_MS_ = 10
	...
	if(rt_web_flag == RTD_ON) rt_web_counter += 10;		// by wook for Web	
...
void printLogo(void)
{
	...
	fprintf(stdout,"  ######################################################\n");
	fprintf(stdout,"  ######                                          ######\n");
	fprintf(stdout,"  ######        VDS Appication by LINUX           ######\n");
	fprintf(stdout,"  ######                                          ######\n");
	fprintf(stdout,"  ######               v2.8                       ######\n");
	fprintf(stdout,"  ######                                          ######\n");
...
init_system()
{
	...
	if(!read_parameter_NAND()){
		...
		if(gstSysConfig.temper[0]>40 \
			||gstSysConfig.temper[0]<20) gstSysConfig.temper[0] = DEFAULT_FAN_TEMP;
		
		if(gstSysConfig.temper[1]>10 \
			||(gstSysConfig.temper[1]+10)<0) gstSysConfig.temper[1] = DEFAULT_HT_TEMP;
		
		if(gstSysConfig.password[0]>=10) gstSysConfig.password[0] = 0;		
		if(gstSysConfig.password[1]>=10) gstSysConfig.password[1] = 0;
		if(gstSysConfig.password[2]>=10) gstSysConfig.password[2] = 0;
		if(gstSysConfig.password[3]>=10) gstSysConfig.password[3] = 0;
	...
	if (lane_evt_flg[i] == EVENT_CREATED)
	{
...
#endif 
		sendRealtimeIndivDataPacket_1(i ,TRUE);
		...
		fprintf(stdout,"%04d %04d/%02d/%02d %02d:%02d:%02d:%02d lane:%02d spd:%05lu occ1:%05lu occ2:%05lu len:%05lu\n",
				cntNumRtData, \
		                       ...
		                       gstCurrData.occupy[gstLaneLink[i].e_loop], gstCurrData.length[i]);
#endif
		sendRealtimeIndivDataPacket_1(i  ,FALSE);
...
//?뚯폆 ?꾨줈洹몃옒諛띿뿉 ?ъ슜???ㅻ뜑?뚯씪 ?좎뼵
 
#define BUF_LEN 128
//硫붿떆吏 ?≪닔?좎뿉 ?ъ슜??踰꾪띁 ?ш린瑜??좎뼵
 
char buffer[BUF_LEN];
struct sockaddr_in server_addr, client_addr;
char temp[20];
int server_fd, client_fd;
//server_fd, client_fd : 媛??뚯폆 踰덊샇
int len, msg_size,swServer;
...
void init_Server(void)
{
 ...
 //   	}
    	...
    	{// ?뚯폆 ?앹꽦
        		fprintf(stdout,"Server : Can't open stream socket\n");
        		return;
    	}
    	 fcntl_setnb(server_fd); /* set nonblock flag */
    	memset(&server_addr, 0x00, sizeof(server_addr));
    	//server_Addr ??NULL濡?珥덇린??
    	...
    	{//bind() ?몄텧
        		fprintf(stdout,"Server : Can't bind local address.\n");
        		return;
    	}
 
    	if(listen(server_fd, 5) < 0)
    	{//?뚯폆???섎룞 ?湲곕え?쒕줈 ?ㅼ젙
        		fprintf(stdout,"Server : Can't listening connect.\n");
        		return;
    	}
 
    	memset(buffer, 0x00, sizeof(buffer));
    	fprintf(stdout,"Server : wating connection request.\n");
 	...
 	switch(swServer)
 	{
 		case 0:
 		{
 			len = sizeof(client_addr);
        			...
        			if(client_fd < 0) break; //{close(client_fd); break;}
...
//       			fprintf(stdout,"Server : %s client connected.\n", temp);
    
       			msg_size = read(client_fd, buffer, 100);
        			if(msg_size < 1) break;	
 			if((buffer[12] == 0xbe) & (waitTime == 0)) {
 				op_time_req(buffer, 0xa2);
 				waitTime = 1000;
 			}
...
static void *network_check_task(void *arg)
{
	...
	while(1)
	{
		...
		else if(0) //.state == SM_VDS_CLIENT_CONNECTED && validsession_counter > 10000)
		{
...
static void *fifo_read_task(void *arg)
{
    ...
    while(1){
		...
		if(msg_size>0) fifo_process_web(fmsg);
	}
}