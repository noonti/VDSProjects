...
/*     1       2       3       4       5       6       7       8     */
/* 01 */	0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7,
/* 02 */	0x8108, 0x9129, 0xa14a, 0xb16b, 0xc18c, 0xd1ad, 0xe1ce, 0xf1ef,
/* 03 */	0x1231, 0x0210, 0x3273, 0x2252, 0x52b5, 0x4294, 0x72f7, 0x62d6,
/* 04 */	0x9339, 0x8318, 0xb37b, 0xa35a, 0xd3bd, 0xc39c, 0xf3ff, 0xe3de,
/* 05 */	0x2462, 0x3443, 0x0420, 0x1401, 0x64e6, 0x74c7, 0x44a4, 0x5485,
/* 06 */	0xa56a, 0xb54b, 0x8528, 0x9509, 0xe5ee, 0xf5cf, 0xc5ac, 0xd58d,
/* 07 */	0x3653, 0x2672, 0x1611, 0x0630, 0x76d7, 0x66f6, 0x5695, 0x46b4,
/* 08 */	0xb75b, 0xa77a, 0x9719, 0x8738, 0xf7df, 0xe7fe, 0xd79d, 0xc7bc,
/* 09 */	0x48c4, 0x58e5, 0x6886, 0x78a7, 0x0840, 0x1861, 0x2802, 0x3823,
...
uint16 updateCRC16(uint8 *data_blk_ptr, uint16 data_blk_size, uint16 crc_accum)
{
	register uint16 i, j;
	...
	for(j=0; j<data_blk_size; j++)
	{
		...
		i = ((unsigned short)(crc_accum) ^ *data_blk_ptr++) & 0xff;
...
uint16 updcrcr(uint16 crc, uint16 c)
{
	uint16 tmp;
...
*/
char
check_crc16ldb (uchar *buf, uint len)
{
	ushort crc, tcrc;
	crc = crc16_ccitt((char *)buf, len);
	tcrc = (buf[len]<<8) + buf[len+1];
