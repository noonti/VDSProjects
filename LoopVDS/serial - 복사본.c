...
void make_traffictask()
{
	...
	for(i=0; i<gucActiveDualLoopNum; i+=2)
	{
		...
		if(t_lane > MAX_LANE_NUM) break;

		if(lane_st[t_lane].status != IN_STS) continue;  // in status 상태
		...
		if (xor_value)
		{
			...
			for(i=0, bit_msk=1; i<gucActiveDualLoopNum; i++, bit_msk<<=1)
			{
					...
					else
					{
						...
						if (lp2ln[i].pos == START_POS)
						{
							...
							{										// 이 루틴은 스타트 포스 확인 조건문에 삽입되어야한다
								...
								if(invers_st.start_inv_flag == FALSE) continue;			//역주행 수집시작  2015.01.07 by capi
								...
								if(lane_st[t_lane].intime[START_POS] > lane_st[t_lane].intime[END_POS]) invers_st.din = 1;
								else invers_st.din = 0;

								if(lane_st[t_lane].time[START_POS] > lane_st[t_lane].time[END_POS]) invers_st.dout = 1;
								else invers_st.dout = 0;
									...
									/*	fprintf(stdout, "\r\n--------------------------------------------------------------------------------------------\r\n");
										...
										*/
										...
										if(gstCurrData.speed[t_lane] < 20000)
										{
											...
											if(rt_web_flag == RTD_ON) saveRealtimeDatatoText(t_lane, TRUE);
										}
									}
				...
				else
				{
					...
					if (cur_value & bit_msk) stuckoff_counter[i] = 0;
					else stuckon_counter[i] = 0;
				}
			}
#if 1
				...
				if (xor_value & bit_msk)
				{
					...
					else // Released // 해당 Loop를 차량을 벗어났다면
					{
						...
						if (loop_st[gucActiveDualLoopNum + i*2].status == PRESSED)
						{
							...
							#if defined(SUPPORT_LOCAL_TRAFFIC_DATA)
								if (loop_gap >= localPollCounter) gstLocalAccuData.occupy[gucActiveDualLoopNum + i*2] = localPollCounter;
								else gstLocalAccuData.occupy[gucActiveDualLoopNum + i*2] += loop_gap;
						...
						// 교통량과 점유시간만 측정하는 용도의 Loop인 경우.
						if ( lp2ln[gucActiveDualLoopNum + i*2].used == UNUSED ) continue;
...
#endif
#if 1  // kiss
			if(vds_st.state == 3)	//2014.03.26 insert by capi 	SM_VDS_CLIENT_DEVID_SEND = 3
			{
				if(invers_st.inverse_flag)
				{
					if(invers_st.inverse_cnt == 0)
					{
						...
						//for (i=0; i<MAX_LANE_NUM; i++) invers_st.gRT_Reverse[i] = FALSE;
					}
					else
					{
