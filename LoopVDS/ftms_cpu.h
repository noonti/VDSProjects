...
#define MAX_LOOP_NUM				32  // avogadro 32
...
// avogadro
typedef struct {
	uint16 total_volume; // 교통량
	uint16 sum_of_speed; // speed 합 (평균속도 구하기 위함)
	uint16  sum_of_occupy; // 점유시간 합
	uint16 speed_category[SPEED_CATEGORY_NO];   // 속도별 교통량
	uint16 length_category[LENGTH_CATEGORY_NO]; // 크기별 교통량
	uint16 polling_time ;
} YSE_REPORT_PER_LANE;
