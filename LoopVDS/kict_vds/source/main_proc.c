...
void printLogo(void)
{
    ...
    fprintf(stdout,"  ######        2021.07.17 (KICT Test        )    ######\n");
    fprintf(stdout,"  ######################################################\n");
    fprintf(stdout,"");
}
    ...
    while(1)
    {
        ...
        */
        ...
        userTermTask();
		  ...
		  //kiss for reverse car- 200525
		  // 20201-06-09 건기연 프로토콜 적용으로 코멘트 처리  by avogadro
		  //InverseDataCheckAndSend();
...
static void *watchdog_task(void *cnt)
{
    ...
    while(1)
    {
        ...
        //if((*wdcnt) > 2000) // 기존 코드
		if ((*wdcnt) > 10000) // 10 초이상 작업이 있을 경우 리셋(2021.07.22 by avogadro)
        {
            ////reset
...
static void *network_check_task(void *arg)
{
    ...
    while(1)
    {
		...
		// 2021-06-09 건기연 프로토콜 적용에 따라 아래 코드는 코멘트 처리 함 by avogadro
        ...
        else if(vds_st.state == SM_VDS_CLIENT_CONNECTED && validsession_counter > 10000)
        {
            ...
            i2cSegDigit(0,2);
#endif
			// 2021-06-09 건기연 프로토콜 적용에 따라 아래 코드 코멘트 처리 by avogadro
