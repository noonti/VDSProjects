...
char ** argv;
{
    ...
    while (argv[optind])
    {
        ...
        while (loport <= curport)
        {
            ...
            {
                x = 0;				/* pre-exit status */
                //holler ("%s [%s] %d (%s) opennnn",
                //	whereto->name, whereto->addrs[0], curport, portpoop->name);
                //holler ("%s %d open", whereto->addrs[0], curport);

                // kwj insert : "open" text print
            ...
            else     /* no netfd... */
            {
                ...
                {
                    //holler ("%s [%s] %d (%s) timeeeeeee",
                    //	whereto->name, whereto->addrs[0], curport, portpoop->name);
                    // kwj modify : print timeout
                    holler ("%s %d  timeout", whereto->addrs[0], curport);
                }
