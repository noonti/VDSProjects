...
void initRealtimePool()
{
    ...
    for(i=0; i<MAX_LANE_NUM; i++)
        lastest_rt_idx[i] = 0;		// by jwank 081218 bug fix.
    ...
    mkdir("/root/am1808/savefolder", mode);	// by jwank 081218 bug fix.
	 ...
	 write_debug_log("program start....initRealtimePool.....");
}
...
void initPollingPool()
{
    ...
    pl_widx = 0;
    pl_data_num = 0;
    for(i=0; i<MAX_SAVE_PL_DATA; i++)
        gstSavePLdataPool[i].valid = 0;
    lastest_pl_idx = 0;
}
...
void saveRealtimeDatatoNand(uint32 t_lane, uint8 inv)
{
    ...
    if (t_lane >= MAX_LANE_NUM)
	 ...
	 //printf("avogadro saveRealtimeDatatoNand..inv=%d \n", inv);
	...
	gstSaveRTdataNAND[nd_bank][rt_nand_widx].lane_occupy = gstCurrData.lane_occupy; // 2021-06-10 건기연 포로토콜 적용  by avogadro

	gstSaveRTdataNAND[nd_bank][rt_nand_widx].lane_occupy_method1 = gstCurrData.lane_occupy_method1; // 2021-06-10 건기연 포로토콜 적용  by avogadro
	gstSaveRTdataNAND[nd_bank][rt_nand_widx].lane_occupy_method2 = gstCurrData.lane_occupy_method2; // 2021-06-10 건기연 포로토콜 적용  by avogadro
	gstSaveRTdataNAND[nd_bank][rt_nand_widx].lane_occupy_method3 = gstCurrData.lane_occupy_method3; // 2021-06-10 건기연 포로토콜 적용  by avogadro
    rt_nand_widx++;
    if (rt_nand_widx >= MAX_SAVE_RT_DATA_NAND)
        rt_nand_widx=0;
    if (rt_nand_data_num < MAX_SAVE_RT_DATA_NAND)
        rt_nand_data_num++;
...
void saveRealtimeData(uint32 t_lane)
{
    ...
    rt_widx++;
    if (rt_widx >= MAX_SAVE_RT_DATA)
        rt_widx=0;
    if (rt_data_num < MAX_SAVE_RT_DATA)
        rt_data_num++;
...
void saveRealtimeDatatoText(uint32 t_lane, uint8 inv)
{
    ...
    if (t_lane >= MAX_LANE_NUM || gstCurrData.length[t_lane] <= 25) //  250 cm 이하는 제외(오토바이등)
        return;
    ...
    if(inv == 1)
    {
        sprintf(rtd_str, "%04d %04d/%02d/%02d %02d:%02d:%02d:%02d 방향:%02d 차로:%02d 속도:-%05lu 점유시간1:%05lu 점유시간2:%05lu 전체점유시간:%05lu 길이:%05lu 점유시간Method1:%05lu \n",
                cntNumRtData % 10000,
				...
				(t_lane%2) + 1, gstCurrData.speed[t_lane],

                (uint16) gstCurrData.occupy[gstLaneLink[t_lane].s_loop],
                (uint16) gstCurrData.occupy[gstLaneLink[t_lane].e_loop],
				(uint16) gstCurrData.lane_occupy, //2021-06-10 건기연 프로토콜 적용 by avogadro
                (uint16) gstCurrData.length[t_lane] ,
				(uint16) gstCurrData.lane_occupy_method1 //,
				//(uint16) gstCurrData.lane_occupy_method2 //,
				//(uint16) gstCurrData.lane_occupy_method3
...
void savePollingData()
{
    ...
    for(j=0; j<MAX_LOOP_NUM/4; j++)
        gstSavePLdataPool[pl_widx].stuck_per_loop[j] = gucStuckPerLoop[j];
    for(j=0; j<MAX_LOOP_NUM/8; j++)
        gstSavePLdataPool[pl_widx].incid_per_loop[j] = incident_per_loop[j];
    ...
    pl_widx++;
    if (pl_widx >= MAX_SAVE_PL_DATA)
        pl_widx=0;
    if (pl_data_num < MAX_SAVE_PL_DATA)
        pl_data_num++;		//debug by capidra MAX_SAVE_RT_DATA->MAX_SAVE_PL_DATA
}
...
void save_log( uint8 err_code, uint8 val)
{
    ...
    if (fsync(fd_log) == -1)
    {
        if(errno == EINVAL)
        {
            if(fdatasync(fd_log) == -1)
                fprintf(stdout, "fdatasync() fail");
            else
...
void write_debug_log(char * szlog)
{
	char logDir[255];
	char logbuff[255];
	char strlog[255];
	...
	sprintf(logbuff, "%s/%04d%02d%02d_debug.log", logDir, sys_time.wYear, sys_time.wMonth, sys_time.wDay);
	//printf("logfile = %s \n", logbuff);
	//fd_log = creat(logbuff, 0666);
	fd_log = open(logbuff, O_WRONLY | O_CREAT | O_APPEND, 0666);
	...
	memset(strlog, 0, 255);
	sprintf(strlog, "[%04d-%02d-%02d %02d:%02d:%02d.%02d]  %s \n",
		sys_time.wYear, sys_time.wMonth, sys_time.wDay,
		sys_time.wHour, sys_time.wMinute, sys_time.wSecond,
		(sys_time.wMilliseconds/10)%100, szlog);
