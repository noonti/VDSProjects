...
static void * sendSavingTrafficDatatoNand(void *arg)
{
    ...
    */
    ...
    while(!isEnd)
    {
		...
		if (gstSaveRTdataNAND[savendBank][ridx].length > 25) // 250 cm 이하는 제외(오토바이등)
		{
			if (gstSaveRTdataNAND[savendBank][ridx].lane < 2)
				direction = 1;// 상행
			else
				direction = 2;
			...
			if (gstSaveRTdataNAND[savendBank][ridx].inverse)
			{
				sprintf(rtd_str, "%04d %04d/%02d/%02d %02d:%02d:%02d:%02d 방향:%02d 차로:%02d 속도:-%05lu 점유시간1:%05lu 점유시간2:%05lu  전체점유시간:%05lu 길이:%05lu 점유시간Method1:%05lu \n",
					...
					(gstSaveRTdataNAND[savendBank][ridx].lane % 2) + 1, gstSaveRTdataNAND[savendBank][ridx].speed,
					(uint16)gstSaveRTdataNAND[savendBank][ridx].occupy[0],
					(uint16)gstSaveRTdataNAND[savendBank][ridx].occupy[1],
					(uint16)gstSaveRTdataNAND[savendBank][ridx].lane_occupy,
					(uint16)gstSaveRTdataNAND[savendBank][ridx].length,
					(uint16)gstSaveRTdataNAND[savendBank][ridx].lane_occupy_method1//,
					//(uint16)gstSaveRTdataNAND[savendBank][ridx].lane_occupy_method2 //,
					//(uint16)gstSaveRTdataNAND[savendBank][ridx].lane_occupy_method3
				);
			}
