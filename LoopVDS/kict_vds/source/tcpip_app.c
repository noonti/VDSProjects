...
//avogadro
int fd_kict_listener;
int fd_kict_ctrl_listener;
int fd_kict_client[1024];
//avogadro
...
int communication_task()
{
	...
	for (i = 0; i < ret_poll; i++)
	{
		...
		{
			/* IS New connection ? */
			// 2021-06-09 건기연 프로토콜 적용으로 코멘트 처리 by avogadro
			...
			if (ep_events[i].data.fd == fd_kict_listener)
			{
				// server mode
				struct sockaddr_storage saddr_c;
				while (1) {
					len_saddr = sizeof(saddr_c);
					fd = accept(fd_kict_listener, (struct sockaddr *)&saddr_c, &len_saddr);
					if (fd == -1) {
						if (errno == EAGAIN) {
							break;
						}
			...
			if (ep_events[i].data.fd == fd_kict_ctrl_listener)
			{
				// server mode
				struct sockaddr_storage saddr_c;
				while (1) {
					len_saddr = sizeof(saddr_c);
					fd = accept(fd_kict_ctrl_listener, (struct sockaddr *)&saddr_c, &len_saddr);
					if (fd == -1) {
						if (errno == EAGAIN) {
							break;
						}
					...
					fcntl_setnb(fd);
					ADD_EV(epollfd, fd);
					pr_out("accept : add ctrl socket (%d)", fd);
					fd_kict_client[fd % 1024] = fd;
			...
			else if ((ep_events[i].data.fd == fd_kict_client[(ep_events[i].data.fd) % 1024])
				     || ep_events[i].data.fd == fd_listener
				     ...
				     )
			{
				...
				if (ret_recv == -1)
				{
					...
					if (ep_events[i].data.fd == fd_listener)
						...
						fd_kict_client[(ep_events[i].data.fd) % 1024] = 0; // reset
				}
				else if (ret_recv == 0)
				{
					/* closed */
				...
				else
				{
					...
					process_KICT_TCP_pkt(ep_events[i].data.fd, (uint8 *)buf, ret_recv);
				}
			}
...
int init_kict_event_socket(int port)
{
	int ret = 1;
	struct addrinfo ai, *ai_ret;
	...
	fd_kict_listener = socket(ai.ai_family, ai.ai_socktype, 0);

	int option;
	option = 1; // SO_REUSEADDR 의 옵션 값을 TRUE 로
	setsockopt(fd_kict_listener, SOL_SOCKET, SO_REUSEADDR, &option, sizeof(option));



	ret = fcntl_setnb(fd_kict_listener);
	if (ret != 0)
	{
		pr_out("fd(%d) init_yse_procotol Error(%d:%s)", fd_kict_listener, errno, strerror(errno));
		return 0;
	}


	addr.sin_family = AF_INET;
	addr.sin_port = htons(port); // 대기 포트
	addr.sin_addr.s_addr = htonl(INADDR_ANY);
	addrlen = sizeof(addr);

	if (bind(fd_kict_listener, (struct sockaddr *)&addr, addrlen) == -1)
		return 0;

	listen(fd_kict_listener, 5);


	ADD_EV(epollfd, fd_kict_listener);

	return 1;

}

int init_kict_ctrl_socket(int port)
{
	int ret = 1;
	struct addrinfo ai, *ai_ret;
	struct sockaddr_in addr;
	...
	fd_kict_ctrl_listener = socket(ai.ai_family, ai.ai_socktype, 0);

	int option;
	option = 1; // SO_REUSEADDR 의 옵션 값을 TRUE 로
	setsockopt(fd_kict_ctrl_listener, SOL_SOCKET, SO_REUSEADDR, &option, sizeof(option));



	ret = fcntl_setnb(fd_kict_ctrl_listener);
	if (ret != 0)
	{
		pr_out("fd(%d) init_yse_procotol Error(%d:%s)", fd_kict_ctrl_listener, errno, strerror(errno));
		return 0;
	}
	...
	if (bind(fd_kict_ctrl_listener, (struct sockaddr *)&addr, addrlen) == -1)
		return 0;

	listen(fd_kict_ctrl_listener, 5);


	ADD_EV(epollfd, fd_kict_ctrl_listener);

	return 1;

}
int init_kict_procotol()
{
	int ret = 1;
	g_isServiceStart = 1;
	ret = init_kict_event_socket(12000);
	ret = init_kict_ctrl_socket(10000);
	return ret;
}
