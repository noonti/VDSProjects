...
//2021-06-10 건기연 프로토콜 적용 by avogadro
uint8 g_isServiceStart = 1; 
...
#if 0 // 스트레스 테스트 위해 by avogadro
	
if (last_send_counter!= global_counter && (global_counter % 1000) == 0)
{
	//printf("send....test packet..\n");
	sendRealtimeDataTest();
	last_send_counter = global_counter;
}
#endif
    ...
    while(evt_buff.rd_ptr != evt_buff.wr_ptr)
    {
        ...
        if (xor_value)
        {
            ...
            // check All IO.
            // 전체 IO를 확인하다
			  //sprintf(szLog, "[make_traffictask] gucActiveDualLoopNum = %d", gucActiveDualLoopNum);
            ...
            for(i=0, bit_msk=1; i<gucActiveDualLoopNum; i++, bit_msk<<=1)
            {
				...
				//2021-06-10 건기연 프로토콜 적용 by avogadro 시작
				// 서비스 시작/중지 요청 받았을 때 변수가 설정되며 서비스 중디 상태일 경우에는 그냥 continue  하여 bypass 한다. 
				if (!g_isServiceStart)
					continue;

				// 2021-06-10 건기연 프로토콜 적용 by avogadro 종료
                    ...
                    else
                    {
                        ...
                        if (lp2ln[i].pos == START_POS)
                        {
                            ...
                            if(lane_st[t_lane].status == IN_RVS)	// IN_RVS인 상태에서 다시 진출 루프를 감지해도 인버스로 들어온다
                            {
                                ...
                                gstCurrData.speed[t_lane] = (360 * 100 * lane_dist[t_lane]) / lane_gap; //  단위: Km/h
                                tmp_len = (lane_dist[t_lane] * loop_gap) / lane_gap - loop_diam[t_lane]; // 단위: dm (1dm = 10 cm)
                                ...
                                if(invers_st.inv_rate < 20)  //역주행관련 4->2
                                {
                                    ...
                                    */
                                    ...
                                    invers_st.inverse_flag = TRUE;	//by capi 2014.09.02
												...
												sprintf(szLog, "[make_traffictask]  inverse state set true. lane=%d , speed = %d ", t_lane, gstCurrData.speed[t_lane]/100);
												write_debug_log(szLog);

												saveRealtimeDatatoNand(t_lane, TRUE); //by avogadro 2021.03.31
                                    ...
                                    if(gstCurrData.speed[t_lane] < 20000)
                                    {
                                        ...
                                        sendRealtimeIndivDataPacket(t_lane, TRUE); //by capi 2014
													 ...
													 sprintf(szLog, "[make_traffictask]  inverse data(lane=%d) send to Maintainance Prg speed(%d) ", t_lane, gstCurrData.speed[t_lane]/100);
													 write_debug_log(szLog);
                                        ...
                                        if(rt_web_flag == RTD_ON)
                                            saveRealtimeDatatoText(t_lane, TRUE);
												...
												{
													sprintf(szLog, "[make_traffictask]  inverse data(lane=%d) not send to Maintainance Prg bacause of speed(%d) ", t_lane, gstCurrData.speed[t_lane]/100);
													write_debug_log(szLog);
                        ...
                        else if(lp2ln[i].pos == END_POS)
                        {
                            ...
                            if (lane_st[t_lane].status == IN_STS)	//2011.09.20 by capidra
                            {
                                ...
                                gstCurrData.speed[t_lane] = (360*100 * lane_dist[t_lane]) / lane_gap;    // 단위: km/h 이지민 소숫점 표현을 위해서 100 을 곱했다. 
                                
								//float fLen = (lane_dist[t_lane] * loop_gap) / lane_gap - loop_diam[t_lane];
								...
								// 점유시간 = 종료 루프의 진출 시간 - 해당 차선의 시작 루프 진입 시간
								gstCurrData.lane_occupy = cur_count - lane_st[lp2ln[i].lane].intime[START_POS]; // 2021-06-10 건기연 프로토콜 적용 by avogadro
								// 점유시간 보정 
								gstCurrData.lane_occupy += 1 + (((gstCurrData.lane_occupy * 15) + 500) / 1000);
								//printf("lane distance = %d , original occupy = %d \n", lane_dist[t_lane] , gstCurrData.lane_occupy);
								// 건기연 시험의 검지 영역은 3m 이다. 점유시간은 loop 간 거리 450 cm 와의 비율로 계산
								float value = (float)gstCurrData.lane_occupy * (30 / (float)(lane_dist[t_lane])); // 루프간 거리 + 루프 지름 + 차량길이
								// 2021 08 05 일단 아래 코드로 실행 gstCurrData.lane_occupy = (uint32)value;



								/* 점유시간 계산 방법1*/
								// 산출된 속도로 이동거리(3M + 차량 길이) 통과 시간 계산
								//gstCurrData.speed[t_lane]  gstCurrData.length[t_lane] 
								// 점유시간(msec) = (36*(이동거리(cm))/속도(Km/h)
								float speedValue = gstCurrData.speed[t_lane] / 100 + (float) (gstCurrData.speed[t_lane] % 100) / 100;
								value = (36 * (300 + gstCurrData.length[t_lane]*10)) / speedValue;// gstCurrData.speed[t_lane]; 차량 길이는 단위가 dm 으로 cm 로 변환 위해 10을 곱했다
								gstCurrData.lane_occupy_method1 = (uint32)value;


								// 2021 08 05 일단 아래 코드로 실행 --> 이 부분..
								gstCurrData.lane_occupy = (uint32)value;

								//printf("method1..value=%f gstCurrData.length[t_lane]=%d  speed=%f \n", value, gstCurrData.length[t_lane], speedValue);
								
								///* 점유시간 계산 방법2*/
								//// 점유시간 = 종료 루프의 진출 시간 - 해당 차선의 시작 루프 진입 시간
								//gstCurrData.lane_occupy_method2 = cur_count - lane_st[lp2ln[i].lane].intime[START_POS]; // 2021-06-10 건기연 프로토콜 적용 by avogadro
								//// 점유시간 보정 
								//gstCurrData.lane_occupy_method2 += 1 + (((gstCurrData.lane_occupy * 15) + 500) / 1000);
								//value = (float)gstCurrData.lane_occupy_method2 * (30 / (float)lane_dist[t_lane]);
								//gstCurrData.lane_occupy_method2 = (uint32)value;


								//printf("method2..value=%d \n", gstCurrData.lane_occupy_method2);

								///* 점유시간 계산 방법3*/
								//// 산출된 속도로 이동거리(3M) 통과 시간 계산
								//// 점유시간(msec) = (36*(이동거리(cm))/속도(Km/h)
								//value = 36 * (300) / speedValue;
								//gstCurrData.lane_occupy_method3 = (uint32)value;

								//printf("method3..value=%f \n", value);

								//printf(" speed= %d ,length=%d, gstCurrData.lane_occupy_method1=%d, gstCurrData.lane_occupy_method2 = %d, gstCurrData.lane_occupy_method3=%d \n", gstCurrData.speed[t_lane], gstCurrData.length[t_lane], gstCurrData.lane_occupy_method1, gstCurrData.lane_occupy_method2, gstCurrData.lane_occupy_method3);
								//printf("lane distance = %d , converted occupy = %d , float occupy =%f lane_gap=%d \n", lane_dist[t_lane], gstCurrData.lane_occupy, value, lane_gap);
                                ...
                                if (isValid == TRUE)
                                {
									...
									//2021-06-09 건기연 프로토콜 적용 by avogadro
									sendRealtimeDataToKICTCenter(t_lane);
									...
									sprintf(szLog, "[make_traffictask] normal car detect. lane=%d, speed=%d", t_lane, speed_integer);
									write_debug_log(szLog);
            ...
            // Single loop ----------------------------
				//sprintf(szLog, "[make_traffictask] gucActiveSingleLoopNum = %d", gucActiveSingleLoopNum);
				//write_debug_log(szLog);
