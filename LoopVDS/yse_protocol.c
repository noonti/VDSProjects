...
int process_sync_command(int fd, YSE_FRAME * request_frame)
{
	...
	printf("frameNo = %d-->0x%04x\n", g_curFrameNo, g_curFrameNo);
...
int process_change_rtc_command(int fd, YSE_FRAME * request_frame)
{
	...
	nextTime.wYear = request_frame->data[0]*100 + request_frame->data[1];// year 
	nextTime.wMonth = request_frame->data[2];
	nextTime.wDay = request_frame->data[3];
	nextTime.wHour = request_frame->data[4];
	nextTime.wMinute = request_frame->data[5];
	nextTime.wSecond = request_frame->data[6];
	...
	//getRtcTime(&currntTime, 0);
	
	memset(data, 0, 7);
	data[0] = currntTime.wYear / 100;
	data[1] = currntTime.wYear % 100;
	data[2] = currntTime.wMonth;
	data[3] = currntTime.wDay;
	data[4] = currntTime.wHour;
	data[5] = currntTime.wMinute;
	data[6] = currntTime.wSecond;
