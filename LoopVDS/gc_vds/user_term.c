...
void init_keyboard()
{
    ...
    for(i=0; i< MAX_USER_TERM_LINE; i++)
        user_term_buff[i] = 0;
...
int gets_alt(char *s, int *size)
{
    ...
    default :
        ...
        usertermcnt++;
        if (usertermcnt > (MAX_USER_TERM_LINE-3))
            break;
    ...
    if(gets_alt(user_term_buff, &cmd_size))
    {
        ...
        case INPUT_TIME_STAT:
            if (cmd != EXIT_CHAR && cmd != NUL)
            {
                ...
                // Month
                tptr = strtok(NULL, "/");
                if ( tptr == NULL || strlen(tptr) > 2 || strlen(tptr) == 0 )
                    goto no_save_time;
                //tmptime.wMonth = atoi(tptr);
                ret = str2num(tptr, (int *)&tmptime.wMonth);
                if (ret == FALSE)
                    goto no_save_time;
                if (tmptime.wMonth > 12 || tmptime.wMonth <= 0)
                    goto no_save_time;
                ...
                if (ret == FALSE)
                    goto no_save_time;
                if (tmptime.wDay > 31 || tmptime.wDay <= 0)
                    goto no_save_time;
...
no_save_time :
        ...
        case INPUT_ADDR_STAT:
            ...
            if (cmd != EXIT_CHAR && cmd != NUL)
            {
                ...
                // Group
                tptr = strtok(user_term_buff, "-");
                if ( tptr == NULL || strlen(tptr) > 5 || strlen(tptr) == 0 )
                    goto no_save_addr;
                tmp_id = atoi(tptr);

                if (tmp_id > 0x0000ffff)
                    goto no_save_addr;
        ...
        case CONFIG_SUBNET_MSK:
            if (cmd != EXIT_CHAR && cmd != NUL)
            {
                ...
                if (status == 0)
                {
                    ...
                    for(i=0; i<4; i++)
                        gstNetConfig.nmsk[i] = subn_msk[i];
        ...
        case CONFIG_SERVER_IP:
            if (cmd != EXIT_CHAR && cmd != NUL)
            {
                ...
                if (status == 0)
                {
                    ...
                    for(i=0; i<4; i++)
                        gstNetConfig.server_ip[i] = server_ip[i];
                    writeNetConfigToNAND(TRUE);
        ...
        */
        ...
        case CONFIG_MAX_LOOP:
            ...
            if (cmd != EXIT_CHAR && cmd != NUL)
            {
                ...
                for(i=0; i<MAX_LANE_NUM; i++)
                    gucDivDualLoopLane[i] = 0;

                dlane = gucActiveDualLoopNum/2;
                for(i=0; i<gucActiveSingleLoopNum; i++)
                    gucDivDualLoopLane[dlane+i] = 1;

                if ( gucActiveLoopNum > MAX_RIO_LOOP_NUM )
                    gucActiveLoopNum = MAX_RIO_LOOP_NUM;
        ...
        case REALTIME_MONITOR_MODE:
        {
            for (i=0; i<MAX_LANE_NUM; i++)
            {
                if (lane_evt_flg[i] == EVENT_CREATED)
                {
                    ...
                    {
                        //Kiss if(gstCurrData.speed[i]<20000)
                        {
#if 1
                            fprintf(stdout,"\n%04d %04d/%02d/%02d %02d:%02d:%02d:%02d", cntNumRtData, \
                                    sys_time.wYear, sys_time.wMonth, sys_time.wDay, \
