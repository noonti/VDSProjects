...
void initVdsGlobalVal ()
{
    ...
    for (i=0; i<MAX_LANE_NUM; i++)
    {
        ...
        for (j=0; j<LENGTH_CATEGORY_NO; j++)
				//gstReportsOfLane[i].speed_category[j] = 0;
            gstReportsOfLane[i].length_category[j] = 0; // bug fix by avogadro(2020.10.03)

		  // 초기화..
		  memset(&gstYSEReportsOfLane[activeYSEReportIndex == 0 ? 1 : 0][i], 0, sizeof(YSE_REPORT_PER_LANE));
...
uint8 insertYSEReportData(uint8 t_lane, uint32  t_speed, uint32 t_length, uint32 occupy)
{
	...
	//speed 
	gstYSEReportsOfLane[activeYSEReportIndex][t_lane].speed_category[speed_class]++;
	...
	if (length_class >= LENGTH_CATEGORY_NO)
		return;
	gstYSEReportsOfLane[activeYSEReportIndex][t_lane].length_category[length_class]++;


	gstYSEReportsOfLane[activeYSEReportIndex][t_lane].total_volume++;
	gstYSEReportsOfLane[activeYSEReportIndex][t_lane].sum_of_occupy += occupy;
	gstYSEReportsOfLane[activeYSEReportIndex][t_lane].sum_of_speed += t_speed;


	//printf("lane=%d , speed=%d , length=%d, occupy=%d speed_class = %d total_volume=%d sum_of_occupy=%d sum_of_speed=%d \n", t_lane, t_speed, t_length, occupy, speed_class, gstYSEReportsOfLane[activeYSEReportIndex][t_lane].total_volume, gstYSEReportsOfLane[activeYSEReportIndex][t_lane].sum_of_occupy, gstYSEReportsOfLane[activeYSEReportIndex][t_lane].sum_of_speed);
                            ...
                            if (lane_st[t_lane].status == IN_STS)	//2011.09.20 by capidra
                            {
                                ...
                                if (isValid == TRUE)
                                {
                                    ...
                                    occupy[t_lane] += loop_gap;
												...
												//용서고속도로 전송 정보 저장 // 2020.10.03 용서고속도로 프로토콜 적용 by avogadro
												insertYSEReportData(t_lane, speed_integer, gstCurrData.length[t_lane], loop_gap);
