...
//	rtc[4] =  byDay & 0x07;
   ...
   if( write(i2c_rtc,rtc,8) == -1)
      fprintf(stdout,"write error errono=%d ,msg=%s \n", errno, strerror(errno));
	else
...
check_time()
{
    ...
    if(oday != sys_time.wDay)
	 {
		  printf("check_time...sync..day oday=%d , sys_time.wDay = %d \n ", oday, sys_time.wDay);
        //RTC and OsTime Synchronization
        getOsTime(&ostime);

		  printf("check_time getosTime : %d-%d-%d %d:%d:%d \n"
			  , ostime.wYear, ostime.wMonth, ostime.wDay
			  , ostime.wHour, ostime.wMinute, ostime.wSecond);
        ...
        while((entry = readdir(dp)) != NULL)
        {
            ...
            if(S_ISDIR(statbuf.st_mode))
            {
                ...
                if(atoi(strmonth)>sys_time.wMonth)
                {
                    ...
                    {
                    case 1:  date = sys_time.wDay +31 - atoi(strday);     break;
                    case 2:  date = sys_time.wDay +28- atoi(strday);     break;
                    case 3:  date = sys_time.wDay +31- atoi(strday);     break;
                    case 4:  date = sys_time.wDay +30- atoi(strday);     break;
                    case 5:  date = sys_time.wDay +31- atoi(strday);     break;
                    case 6:  date = sys_time.wDay +30- atoi(strday);     break;
                    case 7:  date = sys_time.wDay +31- atoi(strday);     break;
                    case 8:  date = sys_time.wDay +31- atoi(strday);     break;
                    case 9:   date = sys_time.wDay +30- atoi(strday);     break;
                    case 10:  date = sys_time.wDay +31- atoi(strday);     break;
                    case 11:  date = sys_time.wDay +30- atoi(strday);     break;
                    case 12:  date = sys_time.wDay +31- atoi(strday);     break;
                    }
    ...
    if(/* !(sys_time.wHour%3) &&*/ (ohour != sys_time.wHour))
    {
        ...
        if(kickRTSavingDatatoNand_m())
        {
            ...
            if( ret != 0) fprintf(stdout,"create saving data task error\n");
        }
        else
        {
    ...
    if(LD_Send_val > 2)
    {
        ...
        if(curID >= gucActiveLDBNum) curID = 0;
