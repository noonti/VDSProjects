...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        */
        ...
        //kiss - 200525
        //InverseDataCheckAndSend();  // 2020.10.03 용서고속도로 프로토콜 수정(역주행 감지 없음) by avogadro
...
static void *network_check_task(void *arg)
{
    char *s=(char *)arg;
    int rtn, ldb,i;
    printf("network_check_task...enter \n");
    while(1)
    {
        ...
        if(validsession_counter > 300000 && validsession_flag == 0)
        {
            ...
            if(vds_st.state == SM_VDS_CLIENT_DEVID_SEND)		//2014.03.26 insert by capi
            {
                ...
                {
                    printf("valid session check...before \n");
                    checkvalidTask();	//유효성 확인
                    printf("valid session check...after \n");
                    validsession_flag = 1;
                }
                else
                {
        ...
        else if(vds_st.state >= SM_VDS_CLIENT_ABORTED)
        {
            ...
            printf("network_check_task...SM_VDS_CLIENT_ABORTED \n");
            if(rtn == TRUE)
            {
#ifdef SEGMENT
                i2cSegDigit(1,1);
                ...
                //init_tcpip(); // 2020.10.03 용서고속도로 프로토콜 수정(제어기가 서버모드) by avogadro
                //init_yse_procotol();
